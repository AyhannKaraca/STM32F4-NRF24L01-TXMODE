...
int main(void)
{
  /* USER CODE BEGIN 1 */
	...
	uint8_t address[] = {0xEE,0xEE,0xEE,0xEE,0xEE};
	uint8_t radio_command[32];
	uint16_t VR[2];
  ...
  if(isOkStart()){
	  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_15, SET);//Status led.
	  NRF24_TxInit(address, 100);
  }
  else{
	  Error();
  }
  ...
  {
	  if(data_ready){
		  radio_command[0] = VR[0] & (0xFF);
		  radio_command[1] = VR[0] >> 8;

		  radio_command[2] = VR[1] & (0xFF);
		  radio_command[3] = VR[1] >> 8;
